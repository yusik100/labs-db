...
- **Опис:** Використовується для знаходження "маленьких" релізів (синглів або EP). Спочатку треки групуються за альбомом, потім `COUNT` рахує їх кількість, а `HAVING` відсіює ті групи, де треків 4 або більше.
...
- **Опис:** Створює список усіх можливих комбінацій користувачів і тарифів, незалежно від реальних покупок. Корисне для моделювання даних.
